...
class Pacgum(Entity):
    def __init__(self, pos: tuple[float, float], size: int, points: int) -> None:
        super().__init__(
            pos,
            size,
            0
        )
        sprite_sheet = SpriteLibrary['blue']
...
class SuperPacgum(Entity):
    ...
    def update(self, dt: float) -> None:
        ...
        if self.__animation_timer >= self.__animation_delay:
            ...
            self.__frame_index = (self.__frame_index + 1) % len(self.__animation)
